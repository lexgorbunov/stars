...
public class Animate extends Bitmap {
    ...
    private var cadrW,cadrH:Number;
    private var timer:Timer;
    private var cadrsCount:uint=0;
    private var pos:uint=0;
    private var transparent:Boolean=false;

    public function Animate(img:BitmapData,gridXCadrCount,gridYCadrCount:uint,transparent:Boolean=true,fps:uint=24,autoStart:Boolean=true,repeatCount:uint=0) {
        cadrW = img.width/gridXCadrCount;
        cadrH = img.height/gridYCadrCount;
        this.transparent=transparent;
        for(var yi=0;yi<gridYCadrCount;yi++)
        {
            for(var xi=0;xi<gridXCadrCount;xi++)
            {
                var cadrBitmap:BitmapData=new BitmapData(cadrW,cadrH,transparent);
//                var m:Matrix=new Matrix();
//                m.createBox(1,1,0,-xi*cadrW,-yi*cadrH);
//                cadrBitmap.draw(img,m,null,null,new Rectangle(0,0,cadrW,cadrH));
...
//        super.bitmapData=cadrs[1];
    ...
    override public function get width():Number{return cadrW;}
    override public function get height():Number{return cadrH;}
}
}
